...
for module in to_delete:
    for file in REPO_APK_DIR.glob(f"tsundoku-{module}-v*.*.*.apk"):
        print(f"removing {file.name}")
        file.unlink(missing_ok=True)
    for file in REPO_JAR_DIR.glob(f"tsundoku-{module}-v*.*.*.jar"):
        print(f"removing {file.name}")
        file.unlink(missing_ok=True)
    for file in REPO_ICON_DIR.glob(f"eu.kanade.tachiyomi.novelextension.{module}.png"):
        print(f"removing {file.name}")
        file.unlink(missing_ok=True)
...
index = index_pb2.Index(
    ...
    contact=index_pb2.Contact(
        website="https://novelsourcery.github.io", discord="https://discord.gg/JG2K2jTjd6"
    ),
